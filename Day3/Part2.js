function sumPartNumbers(schematic) {
    ...
    for (let i = 0; i < schematic.length; i++) {
        let row = schematic[i].split('');
        for (let j = 0; j < row.length; j++) {
            if (!isNaN(row[j]) && row[j] !== '.') {
                let number = '';
                let startJ = j;
                while (j < row.length && !isNaN(row[j]) && row[j] !== '.') {
                    number += row[j];
                    j++;
                }
                ...
                for (let k = startJ; k < j; k++) {
                    for (let direction of directions) {
                        ...
                        let ny = k + direction[1];
                        if (nx >= 0 && nx < schematic.length && ny >= 0 && ny < row.length && symbols.includes(schematic[nx][ny])) {
                            isPartNumber = true;
                            holderArray.push({
                                "cord": nx+''+ny,
                                "number": number
    ...
    const result = holderArray.reduce((acc, {cord, number}) => {
        acc[cord] ??= {cord: cord, number: []};
        if(Array.isArray(number)) // if it's array type then concat 
          acc[cord].number = acc[cord].number.concat(value);
        else
          acc[cord].number.push(number);
      ...
      Object.values(result).forEach(element => {
        if(element.number.length === 2){
            sum += (element.number[0] * element.number[1])
        }
      });
    return sum;
}
...
let schematic = `.......12.......935............184.720...243........589.652..........435..........483.............6...........................904...........
