...
isValidGame = true;
str.forEach(element => {
    createDictionaryForGame(element)
    sum += (currentGreen*currentRed*currentBlue);
});
console.log(sum);

function checkIfValidGame(array){
    currentGreen = 0;
    currentRed = 0;
    currentBlue = 0;
    for(let i = array.length; i >= 0; i--){
        if(isColor(array[i])){
            ...
            while((i-1) >= 0 && !isColor(array[i-1])){
                currentNumberInString = array[i-1];
                i--;
            }
            if(currentColor == 'green'){
                if(parseInt(currentNumberInString) > currentGreen){
                    currentGreen = parseInt(currentNumberInString);
                }
            }
            else if(currentColor == 'red'){
                if(parseInt(currentNumberInString) > currentRed){
                    currentRed = parseInt(currentNumberInString);
                }
            }
            else if(currentColor == 'blue'){
                if(parseInt(currentNumberInString) > currentBlue){ 
                    currentBlue = parseInt(currentNumberInString);
                }
            }
            currentColor = '';
            currentNumberInString = '';
...
function isColor(string){
    if(string == 'green' || string == 'red' || string == 'blue'){
        return true;
    }
    else{
        return false;
...
function createDictionaryForGame(element){
    matches = [];
    isValidGame = true;
    let gamesWithoutStart = element.split(": ").pop().split("; ");
    gamesWithoutStart.forEach(data => {
    previousMatch = '';
    currentMatch = '';
        for(let i = data.length; i >= 0; i--){
            temp = '';
            if(isColor(previousMatch)){
                while(!isNaN(parseInt(data[i-1])) && i > 0){
                    temp = data[i-1] + temp;
                    i--;
                }
                if(temp !==''){
                    matches.unshift(temp);
                    previousMatch = temp[0];
                }
            }
            currentData = data.substring(i, data.length);
            currentMatch = currentData.match(regex);

            if(currentMatch != null && (currentMatch[0] != previousMatch) && temp === ''){
                matches.unshift(currentMatch[0]);
                previousMatch = currentMatch[0];
            }
        }
    });
    checkIfValidGame(matches);
    return(isValidGame);
}

function getValues(){
    return `Game 1: 1 green, 1 blue, 1 red; 1 green, 8 red, 7 blue; 6 blue, 10 red; 4 red, 9 blue, 2 green; 1 green, 3 blue; 4 red, 1 green, 10 blue
